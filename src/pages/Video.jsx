...
function Video() {
  ...
  const relatedList = useMemo(
    () => (relatedVideos?.items || []).filter((video) => String(video.id) !== String(id)).slice(0, 10),
    [id, relatedVideos?.items],
  )
  ...
  const pageSchema = useMemo(() => {
    ...
    return {
      ...
      description: data.data.description || 'ویدیو در اربعین تی وی',
      thumbnailUrl: data.data.thumbnailUrl || data.data.cover_link || data.data.cover,
      uploadDate: data.data.created_at,
      contentUrl: data.data.video_link || data.data.videoUrl,
      embedUrl: typeof window !== 'undefined' ? window.location.href : undefined,
      publisher: {
        '@type': 'Organization',
        name: 'اربعین تی وی',
      },
  ...
  const extractFilenameFromContentDisposition = (cd) => {
    if (!cd) return null
    let matched = cd.match(/filename\*=(?:UTF-8'')?(.+)/i)
  ...
  return (
    <Layout>
      <Seo
        title={`${data.data.title} | اربعین تی وی`}
        description={data.data.description || 'تماشای ویدیو در اربعین تی وی'}
        image={data.data.thumbnailUrl || data.data.cover_link || data.data.cover}
        type="video.other"
        schema={pageSchema}
      />
      <article className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 py-4 sm:py-6">
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-[minmax(0,1fr)_400px]">
          <section>
            ...
            <div className="mt-4 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
              <p className="text-sm sm:text-base text-gray-700 leading-6 sm:leading-7 break-all">{data.data.description}</p>
            </div>
          </section>
          <aside className="hidden lg:block">
            <div className="sticky top-24 space-y-3">
              ...
              {!isRelatedLoading && !relatedList.length ? <p className="text-gray-500 text-sm">ویدیوی مرتبطی یافت نشد</p> : null}
            </div>
          </aside>
        </div>
      </article>
