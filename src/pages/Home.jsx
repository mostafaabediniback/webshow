...
function Home() {
  ...
  const [activeChannelId, setActiveChannelId] = useState(location.state?.channelId || null)
  ...
  const channelsList = useMemo(() => (Array.isArray(channelsData?.items) ? channelsData.items : []), [channelsData?.items])
  const videosList = Array.isArray(videosData?.items) ? videosData.items : []

  const activeChannelName = useMemo(
    () => channelsList.find((channel) => channel.id === activeChannelId)?.name,
    [activeChannelId, channelsList],
  )
  ...
  if (channelsLoading && !channelsData) {
    return (
      <Layout>
        <Seo
          title={`${username ? `${username} | ` : ''}اربعین تی وی`}
          description="مشاهده تازه‌ترین ویدیوها و کانال‌های اربعین تی وی."
        />
  ...
  return (
    <Layout>
      <Seo
        title={`${username ? `${username} | ` : ''}اربعین تی وی`}
        description="پخش آنلاین ویدیوها بر اساس کانال‌های مختلف در اربعین تی وی."
      />
      <section className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 py-6 sm:py-8">
          <header className="sticky top-[57px] sm:top-[61px] md:top-[73px] z-40 -mx-3 sm:-mx-4 md:-mx-6 px-3 sm:px-4 md:px-6 py-3 backdrop-blur-md border-b border-slate-200/80 mb-6">
            <CategoryChips
          ...
          {showInitialLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 rounded-2xl bg-slate-50 border border-slate-200 border-dashed">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-2">{activeChannelId ? 'ویدیویی یافت نشد' : 'شروع کنید!'}</h3>
              <p className="text-slate-600 text-center max-w-md">
                {activeChannelId ? 'در این کانال فعلاً ویدیویی موجود نیست.' : 'کانالی انتخاب کنید تا ویدیوهایش را ببینید.'}
              </p>
            </div>
          ) : (
            <article>
