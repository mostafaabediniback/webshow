    ...
    <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-1 scrollbar-hide -mx-1 px-1 p-2">
      <button
        ...
        className={`flex flex-col items-center gap-2 flex-shrink-0 transition-all duration-200 ${
          activeChannelId === null ? 'scale-105' : 'opacity-75 hover:opacity-100 hover:scale-[1.02]'
        }`}
      >
        <div
          className={`w-14 h-14 rounded-xl overflow-hidden border-2 shadow-sm flex items-center justify-center transition-all duration-200 ${
      ...
      {list.map((channel) => {
        ...
        return (
          ...
          >
            <div
              className={`w-14 h-14 rounded-xl overflow-hidden border-2 shadow-sm transition-all duration-200 ${
                isActive
